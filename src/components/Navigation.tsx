  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 sm:px-6">
        <div className={`flex items-center justify-between transition-all duration-700 rounded-2xl md:rounded-3xl px-4 md:px-6 backdrop-blur-xl border ${
          isScrolled 
            ? 'bg-black/60 border-white/10 h-14 md:h-16 shadow-[0_0_40px_rgba(0,0,0,0.5)]' 
          ...
          <div className="hidden lg:flex items-center gap-1">
             {navItems.map((item) => (
                <motion.a
                  ...
                  href={item.href}
                  whileHover={{ y: -2 }}
                  className={`px-3 md:px-4 py-2 rounded-xl text-[8px] md:text-[9px] tracking-widest uppercase transition-all flex items-center gap-2 group relative overflow-hidden ${
                    activeTab === item.name 
          ...
          <div className="hidden lg:flex items-center gap-3 md:gap-4">
             <motion.a
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="px-4 md:px-5 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white font-display font-black text-[9px] tracking-[0.2em] uppercase hover:border-primary/50 transition-all flex items-center gap-2"
             >
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              ...
              className="lg:hidden mt-3 md:mt-4 p-6 md:p-8 glass-card border border-white/10 rounded-[2rem] md:rounded-[2.5rem] flex flex-col gap-5 md:gap-6 shadow-2xl backdrop-blur-xl"
            >
               {navItems.map((item) => (
                 <a 
                   key={item.name}
                 ...
                 >
                    ...
                    <ChevronRight className="w-4.5 h-4.5 text-muted-foreground group-hover:text-primary transition-all" />
                 </a>
               ))}
               ...
               <a 
                 ...
                 target="_blank"
                 rel="noopener noreferrer"
                 className="w-full py-4 md:py-5 rounded-xl bg-primary text-black font-display font-black tracking-widest uppercase text-sm shadow-xl items-center flex justify-center gap-3"
               >
